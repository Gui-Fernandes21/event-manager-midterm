...
exports.getEventDetails = (req, res, next) => {
	...
	global.db.get(eventQuery, [eventId], (err, event) => {
		if (err) {
          ...
          const formattedPublishedAt = format(publishedAtObj, "MMMM d, yyyy 'at' h:mm a");
...
exports.postEventBooking = (req, res, next) => {
	...
	if (!attendee_name || attendee_name.trim() === "") {
		return res.status(400).render("attendee/event.ejs", {
			...
			event: req.body, // You'd need to refetch the event data here
		});
	}
	...
	global.db.get(eventQuery, [eventId], (err, event) => {
		if (err) {
			...
			global.db.get(availabilityQuery, [eventId], (err, bookings) => {
				if (err) {
					next(err);
				} else {
					const generalBooked = bookings.general_booked || 0;
					...
					if (generalTickets > remainingGeneral || vipTickets > remainingVip) {
						return res.status(400).render("attendee/event.ejs", {
							title: `${event.title} - Event Details`,
							event: event,
							remainingTickets: {
